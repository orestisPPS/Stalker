...
struct MathTraits<float, unrollFactor> {
    ...
    static constexpr inline void add(const T_simd* a, const T_simd* b, T_simd* result, const T_simd* scale1, const T_simd* scale2) {
        _unrollAdd<unrollFactor>(a, b, result, scale1, scale2);
    }

    static constexpr inline void subtract(const T_simd* a, const T_simd* b, T_simd* result, const T_simd* scale1, const T_simd* scale2) {
        _unrollSubtract<unrollFactor>(a, b, result, scale1, scale2);
    }

    static constexpr inline void multiply(const T_simd* a, const T_simd* b, T_simd* result, const T_simd* scale1, const T_simd* scale2) {
        _unrollMultiply<unrollFactor>(a, b, result, scale1, scale2);
    }
    
    static constexpr inline void scale(T_simd* data, const T_simd* scale){
        _scale<unrollFactor>(data, scale);
    }

    static constexpr inline void sum(const T_simd *data, T_data *result){
        _sum<unrollFactor>(data, result);
    }

    static constexpr inline void dotProduct(const T_simd* a, const T_simd* b, T_data * result){;
        _dotProduct<unrollFactor>(a, b, result);
    }

private:
    template<unsigned iUnroll>
    static constexpr inline void _unrollAdd(const T_simd* a, const T_simd* b, T_simd* result, const T_simd* scale1, const T_simd* scale2) {
    ...
    template<unsigned iUnroll>
    static constexpr inline void _unrollSubtract(const T_simd* a, const T_simd* b, T_simd* result, const T_simd* scale1, const T_simd* scale2) {
        if constexpr (iUnroll > 0) {
    ...
    template<unsigned iUnroll>
    static constexpr inline void _unrollMultiply(const T_simd* a, const T_simd* b, T_simd* result, const T_simd* scale1, const T_simd* scale2) {
        if constexpr (iUnroll > 0) {
    ...
    template<unsigned iUnroll>
    static constexpr inline void _scale(T_simd* data, const T_simd* scale){
        if constexpr (iUnroll > 0) {
    ...
    template<unsigned iUnroll>
    static constexpr inline void _sum(const T_simd* data, T_data* result) {
        if constexpr (iUnroll > 0) {
    ...
    template<unsigned iUnroll>
    static constexpr inline void _dotProduct(const T_simd* a, const T_simd* b, T_data* result) {
        if constexpr (iUnroll > 0) {
            T_simd temp = _mm256_mul_ps(*(a + iUnroll - 1), *(b + iUnroll - 1));
...
private:
    ...
    static constexpr inline void _dotProduct(const T_simd* a, const T_simd* b, T_data* result) {
        if constexpr (iUnroll > 0) {
            T_simd temp = _mm256_mul_pd(*(a + iUnroll - 1), *(b + iUnroll - 1));
...
private:
    ...
    static constexpr inline void _dotProduct(const T_simd* a, const T_simd* b, T_data* result) {
        if constexpr (iUnroll > 0) {
            T_simd temp = _mm256_mullo_epi32(*(a + iUnroll - 1), *(b + iUnroll - 1));
...
private:
    ...
    static constexpr inline void _dotProduct(const T_simd* a, const T_simd* b, T_data* result) {
        if constexpr (iUnroll > 0) {
            T_simd temp = _mm256_mullo_epi16(*(a + iUnroll - 1), *(b + iUnroll - 1));
