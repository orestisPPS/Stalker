...
def parse_log_content(content):
    """
    Parses the log content into a dictionary containing parameters, timers, and data.
    This function ensures that all numerical values are stored as floats (doubles).
    """
    data = {
        "Parameters": {},  # Parameters are stored as strings, not analyzed statistically.
    ...
    for line in line_iter:
        ...
        if line.startswith('#Region'):
            ...
            entry_value = next(line_iter).strip()  # Keep parameters as strings.
            ...
            entry_value = float(next(line_iter).strip())  # Convert to float immediately.
